 ...
 private:
  void Run() {
    ...
    for (auto name_sit_pair : GetBenchmarkSituations()) {
      std::cout << std::endl
                << "Situation " << name_sit_pair.first << std::endl;
      {
        using namespace std::chrono;
        Negamax<10, 12> negamaxer;
        Situation<10, 12> sit = name_sit_pair.second;
  ...
  Situation<10, 12> ParseSituationOrCrash(std::string standard_notation) {
    Situation<10, 12> sit;
  ...
  std::vector<std::pair<std::string, Situation<10, 12>>>
  GetBenchmarkSituations() {
    return {
        {"Start-position", Situation<10, 12>{}},
  ...
  void PrintRuntimeSettings() {
    ...
    std::cout << "Board dimensions: " << 10 << " x " << 12 << std::endl;
    std::cout << "Negamax depth: " << kMaxDepth << std::endl;
    std::cout << "Branching factor: " << MaxNumLegalMoves(10, 12) << std::endl;
    std::cout << "Sizes (bytes): int: " << sizeof(int)
              << " Graph: " << sizeof(Graph<10, 12>)
              << " Situation: " << sizeof(Situation<10, 12>)
  ...
  void PrintTranspositionTableSummary() {
    ...
    for (int depth = kMaxDepth; depth >= 0; --depth) {
      ...
      table2.AddToLastRow("|");
      {
        long long exact_hits =
            benchmark_metrics.num_exits[depth][TABLE_HIT_EXIT];
        ...
        long long total_hits =
            exact_hits + bound_improvement_hits + useless_hits;
        long long no_checks =
            benchmark_metrics.num_exits[depth][GAME_OVER_EXIT] +
      ...
      }
      {
        ...
        long long no_writes =
            total_visits - insertions - displacements - updates;
