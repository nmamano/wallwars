...
import {
  ...
  MenuColorTheme,
  BoardColorTheme,
} from "../shared/colorThemes";

// Stateless component to display the board. All the state is in GamePage.
export default function Board({
  ...
  playerPos: [Pos, Pos];
  goals: [Pos, Pos];
  handleClick: (pos: Pos) => void;
  creatorToMove: boolean | null;
  groundSize: number;
  wallWidth: number;
  ...
  const [hoveredCell, setHoveredCell] = useState<Pos | null>(null);
  const handleMouseEnter = (pos: Pos) => {
    setHoveredCell(pos);
  };
  ...
  // Cell highlighting.
  const [highlightedCells, setHighlightedCells] = useState<Pos[]>([]);
  const handleClickOrHighlight = (
    pos: Pos,
  ...
  const getCol = (elem: keyof MenuColorTheme | keyof BoardColorTheme) =>
    getColor(boardTheme, elem, isDarkModeOn);
  ...
  const isOneOf = (pos: Pos, actions: (Pos | null)[]) => {
  ...
  return (
    ...
    >
      {allPos.map((pos) => {
        ...
        if (cellType === CellType.wall) className += "waves-effect waves-dark";
        ...
        if (cellType === CellType.ground) {
          ...
          if (solidWallHere) {
            color = getCol(grid[pos[0]][pos[1]] === 1 ? "wall1" : "wall2");
          } else if (ghostHere) {
        ...
        if (cellType !== CellType.pillar) style.cursor = "pointer";
        if (cellType === CellType.wall && lastMoveHere) {
        ...
        return (
          <div
            ...
            style={style}
            onClick={
              (cellType === CellType.wall || cellType === CellType.pillar) &&
              handleClick !== null
                ? (event) => {
          ...
          >
            ...
            {cellType === CellType.ground && (shadow1Here || shadow2Here) && (
              <i
